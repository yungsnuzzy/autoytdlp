...
for /d %%C in ("%channelsRoot%\*") do (
    ...
    echo.
    set /p proceed="Proceed to download for %%~nxC? (Y/N): "
    if /i not "!proceed!"=="Y" (
        echo Skipping %%~nxC...
        echo.
        REM Do not use goto here; just continue the loop
    ) else (
        "C:\Program Files\ytdlp\yt-dlp.exe" --output "!channelFolder!\%%(title)s - %%(upload_date>%%Y-%%m-%%d)s.%%(ext)s" --format "!format!" --merge-output-format mp4 --dateafter "!startDate!" "!channelLink!" --add-metadata
    )
    echo.
    echo --------------------------------------------------------------------------------
)
